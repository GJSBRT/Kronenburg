...
function chemicalsmenucheck()
	...
	ESX.TriggerServerCallback('esx_retropluk:CheckLisense', function(cb)
		if cb then
			wasOpen = true
			OpenChemicalsMenu()
		else
...
function OpenChemicalsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'ChemicalsConvertionMenu', {
		...
		menu.close()
		menuOpen = false
		TriggerServerEvent('esx_retropluk:ChemicalsConvertionMenu', data.current.name, 1)
...
--						TriggerServerEvent('esx_retropluk:pickedUpChemicals')
					else
