...
RegisterCommand('rolstoel', function()
    if ESX.PlayerData.job.name ~= 'ambulance' then
        TriggerEvent("chatMessage", "SYSTEM", {255, 0, 0}, "Alleen ambulanciers kunnen een rolstoel pakken!")
        return
    end
	LoadModel(GetHashKey('prop_wheelchair_01'))

	local wheelchair = Citizen.InvokeNative(0x509D5878EB39E842, GetHashKey('prop_wheelchair_01'), GetEntityCoords(PlayerPedId()), true)
	SetModelAsNoLongerNeeded(GetHashKey('prop_wheelchair_01'))
end, false)

function DeleteWheelChair(source, args, raw)
	local wheelchair = GetClosestObjectOfType(GetEntityCoords(PlayerPedId()), 10.0, GetHashKey('prop_wheelchair_01'))

	if DoesEntityExist(wheelchair) and NetworkGetEntityIsNetworked(wheelchair) then
		DeleteEntity(wheelchair)
	end
end

RegisterCommand('drolstoel', DeleteWheelChair)
	...
	while true do
		...
		if DoesEntityExist(closestObject) then
			...
			if #(pedCoords - pickupCoords) <= 1.0 then
				DrawText3Ds(pickupCoords, "[E] Duw rolstoel", 0.4)
...
Sit = function(wheelchairObject)
	...
	if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'missfinale_c2leadinoutfin_c_int', '_leadin_loop2_lester', 3) then
			ShowNotification("Er zit al iemand op de rolstoel!")
			return
		end
	end
...
PickUp = function(wheelchairObject)
	...
	if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'anim@heists@box_carry@', 'idle', 3) then
			ShowNotification("Iemand rijd al met de rolstoel!")
			return
		end
	end
