...
AddEventHandler('esx_jobs:action', function(job, zone)
	...
	elseif zone.Type == "work" then
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			TriggerServerEvent('esx_jobs:startWork', zone.Item, securityToken)
		end
							...
							if playerPed == driverPed then
								...
								for i = 1, #myPlate, 1 do
									if myPlate[i] == plate then
										...
										local vehicleMaxHealth = GetVehicleEngineHealth(vehicleInCaseofDrop)
...
function spawnVehicle(spawnPoint, vehicle, vehicleCaution)
	...
	hintIsShowed = false
	local securityToken = '25291'
	TriggerServerEvent('esx_jobs:caution', 'take', vehicleCaution, spawnPoint, vehicle, securityToken)
end
...
AddEventHandler('esx_jobs:spawnJobVehicle', function(spawnPoint, vehicle)
	...
	ESX.TriggerServerCallback('esx_jobs:checkVehicleDB', function(isFound)
		if isFound then
			ESX.TriggerServerCallback('esx_jobs:getVehicle', function(plate)
				...
				ESX.Game.SpawnVehicle(vehicle.Hash, spawnPoint.Pos, spawnPoint.Heading, function(spawnedVehicle)
					...
					vehicleProps.plate = kenteken



					SetVehicleNumberPlateText(spawnedVehicle, kenteken)
					table.insert(myPlate, kenteken)
					--plate = string.gsub(plate, " ", "")
					...
					exports['KB_server']:spawnedPersonalVehicle(spawnedVehicle)
			...
			ESX.Game.SpawnVehicle(vehicle.Hash, spawnPoint.Pos, spawnPoint.Heading, function(spawnedVehicle)
				...
				TriggerServerEvent('esx_jobs:setJobVehicle', plate)
...
function GeneratePlate()
	...
	while true do
		...
		generatedPlate = 'WORK' .. math.random(100, 9000)


		ESX.TriggerServerCallback('esx_jobs:isPlateTaken', function(isPlateTaken)
