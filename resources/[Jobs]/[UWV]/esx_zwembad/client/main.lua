...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		TriggerServerEvent('esx_poolcleaner:GiveItem', securityToken)
		StartNPCJob()
		Done = true
	end
			...
			for i=1, #elements, 1 do
				...
				ESX.TriggerServerCallback('esx_jobs:checkVehicleDB', function(isFound)
					...
					--					TriggerServerEvent('esx_vehiclelock:registerkeyjob', name, plate, 'no')
										TriggerServerEvent('esx_poolcleaner:caution', 'take', Config.Caution)
									end)
							end)
						else
				...
				if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and PlayerData.job ~= nil then
					...
					if PlayerData.job.name == Config.nameJob then
						...
						if CurrentAction == 'Sale' then
							TriggerServerEvent('esx_poolcleaner:startSale', securityToken)
						end
