...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		TriggerServerEvent('esx_oceansalvage:GiveItem', securityToken)
		StartNPCJob()
		Done = true
	end
end
	...
	while true do
		...
		if NPCTargetPool ~= nil then
			...
			if GetDistanceBetweenCoords(coords, zone.Pos.x, zone.Pos.y, zone.Pos.z, true) < 10 then
                ...
                ESX.Game.Utils.DrawText3D({x = zone.Pos.x, y = zone.Pos.y, z = zone.Pos.z}, 'Druk op  [~g~E~w~] om het lasapparaat te gebruiken', 0.9)
			...
			elseif GetDistanceBetweenCoords(coords, zone.Pos.x, zone.Pos.y, zone.Pos.z, true) < 25 then
				ESX.Game.Utils.DrawText3D({x = zone.Pos.x, y = zone.Pos.y, z = zone.Pos.z}, 'Wrakstuk', 2.7)
			end
		end
	end
end)
			...
			if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and ESX.PlayerData.job ~= nil then
				...
				if ESX.PlayerData.job.name == Config.JobName then
					...
					elseif CurrentAction == 'vente' then
						...
						TriggerServerEvent('esx_oceansalvage:startVente', securityToken)
...
--									TriggerServerEvent('esx_vehiclelock:vehjobSup', plate, 'no')
									...
									RemoveBlip(Blips['NPCTargetPool'])
									Onjob = false
									SetEntityCoords(playerPed, -1609.45, 5255.26, 3.47, true, false, false, false)
								end
							end
						else
