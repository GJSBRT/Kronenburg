...
AddEventHandler('esx_securoservjob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'securoserv' then
		print(('esx_securoservjob: %s attempted to confiscate!'):format(xPlayer.identifier))
		return
	end
...
AddEventHandler('esx_securoservjob:drag', function(target)
	...
	if xPlayer.job.name == 'securoserv' then
		...
		print(('esx_securoservjob: %s attempted to drag (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_securoservjob:putInVehicle', function(target)
	...
	if xPlayer.job.name == 'securoserv' then
		...
		print(('esx_securoservjob: %s attempted to put in vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)
...
RegisterServerEvent('esx_securoservjob:getStockItem')
AddEventHandler('esx_securoservjob:getStockItem', function(itemName, count)
	local _source = source
...
ESX.RegisterServerCallback('esx_securoservjob:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not selectedWeapon then
		print(('esx_securoservjob: %s attempted to buy an invalid weapon.'):format(xPlayer.identifier))
		cb(false)
	else
		-- Weapon
		if type == 1 then
		...
		elseif type == 2 then
			...
			if component then
				...
				print(('esx_securoservjob: %s attempted to buy an invalid weapon component.'):format(xPlayer.identifier))
				cb(false)
			end
		end
	end
...
ESX.RegisterServerCallback('esx_securoservjob:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name, type)

	-- vehicle model not found
	if price == 0 then
		print(('esx_securoservjob: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	else
		if xPlayer.getMoney() >= price then
			xPlayer.removeMoney(price)
...
ESX.RegisterServerCallback('esx_securoservjob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			...
			if rowsChanged == 0 then
				print(('esx_securoservjob: %s has exploited the garage!'):format(xPlayer.identifier))
				cb(false)
			else
				cb(true, foundNum)
			end
