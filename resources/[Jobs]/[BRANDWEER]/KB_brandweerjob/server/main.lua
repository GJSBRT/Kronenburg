...
if Config.MaxInService ~= -1 then
  TriggerEvent('esx_service:activateService', 'brandweer', Config.MaxInService)
end
...
TriggerEvent('esx_phone:registerNumber', 'brandweer', _U('alert_fire'), true, true)
TriggerEvent('esx_society:registerSociety', 'brandweer', 'brandweer', 'society_brandweer', 'society_brandweer', 'society_brandweer', {type = 'public'})

RegisterServerEvent('esx_firejob:giveWeapon')
AddEventHandler('esx_firejob:giveWeapon', function(weapon, ammo)
  local xPlayer = ESX.GetPlayerFromId(source)

  if xPlayer.job.name == 'brandweer' then
    xPlayer.addWeapon(weapon, ammo)
  else
    print(xPlayer.getName() .. ' (License: '.. xPlayer.identifier .. ') wilde een wapen ('.. weapon ..') inspawnen via brandweerjob.')
  end
end)
