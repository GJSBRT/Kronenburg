...
Citizen.CreateThread(function()
	while true do
		...
		if NetworkIsPlayerActive(PlayerId()) then
			...
			if currentWeaponHash ~= nil and currentWeaponHash ~= 0 and currentWeaponHash ~= `WEAPON_UNARMED` and currentWeaponHash ~= `GADGET_PARACHUTE` and currentWeaponHash ~= `WEAPON_PETROLCAN` then
				if currentWeaponHash ~= weaponhashOld then
                    for k,v in pairs(Config.Weapons) do
                        if GetHashKey(v.name) == currentWeaponHash then
						    weaponhashOld = currentWeaponHash
						    ESX.TriggerServerCallback('drgnv_weaponcheck:server:callback', function(result)
						    end, v.name)
                        end
					end
				end
