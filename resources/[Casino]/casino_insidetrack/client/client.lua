...
Citizen.CreateThread(function()
  while ESX == nil do
    ...
    ESX.TriggerServerCallback('insidetrack:server:getVerifyToken', function(token, eventname)
        verifyToken = token
        ...
        while insideTrackActive do
            ...
            if IsControlJustPressed(2, 237) then
                ...
                if (clickedButton == 10) then
                    ...
                    TriggerServerEvent("insidetrack:server:placebet", Utils.CurrentBet, verifyToken)
                    Utils:StartRace()
                ...
                while checkRaceStatus do
                    ...
                    if (raceFinished) then
                        ...
                        if (Utils.CurrentHorse == Utils.CurrentWinner) then
                            TriggerServerEvent("insidetrack:server:winnings", Utils.CurrentGain, verifyToken)
                        end
    ...
    while true do
        ...
        if dist <= 20.0 then
            ...
            if dist <= 1.0 and not insideTrackActive then
				DisplayHelpText("[~g~E~w~] Openen - [~r~BACKSPACE~w~] Verlaten")
                insideMarker = true
            end
        else
            insideMarker = false
