...
cfg.blackjackTables = {
    --[id] = {x,y,z,heading}
    [0] = {
        dealerPos = vector3(-1826.6718, -1195.2598, 14.3363),
        dealerHeading = 189.3691,
        tablePos = vector3(-1826.6718, -1195.2598, 14.3363),
        tableHeading = 189.3691,
        distance = 500.0,
            ...
            if closestChair ~= nil and closestChairDist < 2 then
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then 
                        drawNativeNotification("Druk op ~INPUT_PICKUP~ om blackjack te starten")
                    else 
                        drawNativeNotification("Stoel bezet.")
                    end
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
            ...
            AddTextComponentString("Jouw set ["..tostring(currentHand).."], de set van de dealer ["..tostring(dealersHand).."]")
        ...
        if closestChair == chairId then
            ...
            Citizen.CreateThread(function()
                ...
                if not standOrHitThisRound and sittingAtBlackjackTable then
                    ...
                    TriggerServerEvent("Blackjack:standBlackjack",globalGameId,globalNextCardCount)
                    declineCard()
                    drawNativeNotification("Je reactie duurde te lang! Oen!")
                end
            end)
        else 
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    drawNativeNotification("Wachten op de volgende ronde...")
    blackjackAnimsToLoad = {
      "anim_casino_b@amb@casino@games@blackjack@dealer",
      "anim_casino_b@amb@casino@games@shared@dealer@",
      "anim_casino_b@amb@casino@games@blackjack@player",  
