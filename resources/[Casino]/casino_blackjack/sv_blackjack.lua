...
DiscordWebhook = "https://discord.com/api/webhooks/971711931234807838/Hu_DphOcHagZoWjXyAbMoKLGNUrhVEKB9bcLdOFOom04Em9u-5sq9n8D00QQZPfukrYJ"
            ...
            if tonumber(betAmount) then
                ...
                if betAmount > 0 then
                    if(betAmount > 49999) then
                        TriggerClientEvent("blackjack:notify",source,"~r~Het inzet bedrag is te hoog!")
                        return
                    end
                    if tryTakeChips(source,betAmount) then
                        ...
                        Geschiedenis[source].inzet =  Geschiedenis[source].inzet + betAmount
                        xPlayer = ESX.GetPlayerFromId(source)

                        logMessage('Inzet', xPlayer.getName() .. ' [ID: '.. source..'] heeft '.. betAmount .. 'casino chips ingezet.  \n\nTotaal gewonnen/verloren: '..(Geschiedenis[source].winst - Geschiedenis[source].inzet), 14921392, source)
                        ...
                        TriggerClientEvent("blackjack:notify",source,"~g~Inzet geplaatst t.w.v.: " .. tostring(betAmount) .. " chips.")
                    else 
                        TriggerClientEvent("blackjack:notify",source,"~r~Niet genoeg chips!")
                    end
                end
            end
        end
                                        ...
                                        if currentHand ~= nil then
                                            if currentHand <= 21 then
                                                ...
                                                if dealerHand > 21 then
                                                    ...
                                                    Geschiedenis[source].winst = Geschiedenis[source].winst + potentialWinAmount
                                                    xPlayer = ESX.GetPlayerFromId(source)
                                                    logMessage('Win', xPlayer.getName() .. ' [ID: '.. source..'] heeft **'.. potentialWinAmount .. '** casino chips gewonnen 🎇.  \n\nTotaal gewonnen/verloren: '..(Geschiedenis[source].winst - Geschiedenis[source].inzet), 2617968, source)
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', -1, "💎 Zutphen Casino | " .. xPlayer.getName() .. " heeft " .. tostring(getMoneyStringFormatted(potentialPushAmount)) .. " casino chips gewonnen!")
                                                            end
                                                        end
                                                    end
