    ...
    while true do
        ...
        local PlayerPos = GetEntityCoords(PlayerPed)
            ...
            if dist < 20 then
                ...
                if dist < 2 then
                    DisplayHelpText("Druk op ~INPUT_PICKUP~ om uw casino account te weergeven ~b~")
...
RegisterNUICallback('casino_withdrawl', function(data)
	local cutForBalance = 0
	local price = data.amountw
	TriggerServerEvent('casino:withdraw', tonumber(data.amountw), tonumber(price))
end)
