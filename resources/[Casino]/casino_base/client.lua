...
local carOnShow = GetHashKey('urus')
...
Citizen.CreateThread(function()
	...
	while true do
		...
		local distance = GetDistanceBetweenCoords(PlayerCoords, vector3(1100.3533, 219.5891, -48.7487), 0)
		if distance < 4.0 then
			ESX.ShowNotification('Gedraag je. Erg dat ik dit erin moet zetten. - Cannab')
			SetEntityCoords(PlayerPedId(-1), vector3(1094.3177, 215.3947, -48.9998))
    ...
    while true do
        ...
        for _,location in ipairs(positions) do
			...
			if CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, loc1.x, loc1.y, loc1.z, 2) then 
				DrawText3Ds(loc1.x, loc1.y, loc1.z + 0.3, '[~g~E~s~] Betreden')
			...
			elseif CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, loc2.x, loc2.y, loc2.z, 2) then
				DrawText3Ds(loc2.x, loc2.y, loc2.z + 0.8, '[~g~E~s~] Verlaten')
                if IsControlJustReleased(1, key_to_teleport) then
					DoScreenFadeOut(500)
    				Wait(500)
