...
if closestPlayer == -1 or closestDistance > 2.0 then 
  ...
  if not HaveBagOnHead then
    ...
    ESX.ShowNotification('~g~Je zette een zak op het hoofd van ~w~' .. GetPlayerName(closestPlayer))
    TriggerServerEvent('esx_worek:closest')
    TriggerServerEvent('esx_handcuff:jute')
  else
    ESX.ShowNotification('~r~Deze persoon heeft al een zak op zijn hoofd')
  end
end
...
function OpenBagMenu() --This function is menu function
    ...
    local elements = {
          {label = 'Doe zak op hoofd', value = 'puton'},
