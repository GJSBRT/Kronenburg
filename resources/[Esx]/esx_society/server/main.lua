...
MySQL.ready(function()
	...
	for i=1, #result2, 1 do
		if result2[i] ~= nil and result2[i].job_name ~= nil and result2[i].grade and Jobs ~= nil and Jobs[result2[i].job_name] ~= nil then 
			Jobs[result2[i].job_name].grades[tostring(result2[i].grade)] = result2[i]
		end
	end
end)
DiscordWebhook = "https://discord.com/api/webhooks/973262964431933480/E5uVRbjbP77iaYffgFV0sRFpp_wMoAhfQL_TmkQxAw_wA5SLYzY2OuGbIVBn-qFsEY2D"
	...
	if xPlayer.job.name == society.name then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				...
				logMessage("Kasroof", xPlayer.getName() .. ' ['.. Job.grade_label..'] heeft zojuist **€' .. amount .. '** uit de kluis van: **'.. societyName .. '** gepakt', 16724527, xPlayer.source)
	...
	if xPlayer.job.name == society.name then
		if amount > 0 and xPlayer.getMoney() >= amount then
			TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
				...
				logMessage("Geld toegevoegd", xPlayer.getName() .. ' ['.. Job.grade_label..'] heeft zojuist **€' .. amount .. '** in de kluis van: **'.. societyName .. '** gelegd', 65280, xPlayer.source)
