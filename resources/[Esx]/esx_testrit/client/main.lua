...
local Keys = {
  ...
  Citizen.CreateThread(function ()
	  ...
	  ESX.TriggerServerCallback('esx_testrit:getCategories', function (categories)
		  Categories = categories
	  end)
  
	  ESX.TriggerServerCallback('esx_testrit:getVehicles', function (vehicles)
		  Vehicles = vehicles
	  end)
  ...
  RegisterNetEvent('esx_testrit:sendCategories')
  AddEventHandler('esx_testrit:sendCategories', function (categories)
	  Categories = categories
  end)
  
  RegisterNetEvent('esx_testrit:sendVehicles')
  AddEventHandler('esx_testrit:sendVehicles', function (vehicles)
	  Vehicles = vehicles
  end)
  ...
  function ReturnVehicleProvider()
	  ESX.TriggerServerCallback('esx_testrit:getCommercialVehicles', function (vehicles)
		  local elements = {}
		  local returnPrice
		  ...
		  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			  ...
			  TriggerServerEvent('esx_testrit:returnProvider', data.current.value)
  ...
  function OpenShopMenu()
	  ...
	  for i=1, #Vehicles, 1 do
		  if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			  ...
			  print(('esx_testrit: vehicle "%s" does not exist'):format(Vehicles[i].model))
		  end
	  end
		  ...
		  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'testdrive' then
				if Config.Testdrive == true then
					...
					-- Unfreeze player and set player to visible
					FreezeEntityPosition(playerPed, false)
					SetEntityVisible(playerPed, true)

					-- Testdrive start message
					exports['mythic_notify']:DoHudText('success', _U('start_testdrive'))

					-- Wait 0,5 sec for getting testdrive vehicle
					Citizen.Wait(500)
					
					-- Get testdrive vehicle
					local testdrivevehicle = ESX.Game.GetClosestVehicle()


					-- Testdrive first warning
					Citizen.Wait(60000)
					exports['mythic_notify']:DoHudText('inform', _U('testdrive_warning'))

					-- Wait for testdrive first min
					Citizen.Wait(60000)

					ESX.Game.DeleteVehicle(testdrivevehicle)

					-- Testdrive stop message
					exports['mythic_notify']:DoHudText('inform', _U('stop_testdrive'))

					-- Spawn back at cardealer
					...
					exports['mythic_notify']:DoHudText('error', _U('testdrive_disabled'))
				end
			...
			elseif data2.current.value == 'yes' then
				  if Config.EnablePlayerManagement then
					  ESX.TriggerServerCallback('esx_testrit:buyVehicleSociety', function(hasEnoughMoney)
					  ...
					  if Config.EnableSocietyOwnedVehicles and playerData.job.grade_name == 'boss' then
						  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm_buy_type', {
							  ...
							  if data3.current.value == 'personnal' then
								  ...
								  ESX.TriggerServerCallback('esx_testrit:buyVehicle', function(hasEnoughMoney)
									  if hasEnoughMoney then
										  IsInShopMenu = false
										  ...
										  ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
											  ...
											  if Config.EnableOwnedVehicles then
												  TriggerServerEvent('esx_testrit:setVehicleOwned', vehicleProps)
											  end
								  ...
								  ESX.TriggerServerCallback('esx_testrit:buyVehicleSociety', function (hasEnoughMoney)
									  if hasEnoughMoney then
										  ...
										  ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
											  ...
											  vehicleProps.plate = newPlate
											  SetVehicleNumberPlateText(vehicle, newPlate)
											  TriggerServerEvent('esx_testrit:setVehicleOwnedSociety', playerData.job.name, vehicleProps)
											  ESX.ShowNotification(_U('vehicle_purchased'))
										  end)
  ...
  function OpenGetStocksMenu()
	  ESX.TriggerServerCallback('esx_testrit:getStockItems', function (items)
		  local elements = {}
		  ...
		  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			  ...
			  ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				  ...
				  if count == nil then
					  ...
					  TriggerServerEvent('esx_testrit:getStockItem', itemName, count)
					  menu2.close()
					  menu.close()
					  OpenGetStocksMenu()
				  end
  ...
  function OpenPutStocksMenu()
	  ESX.TriggerServerCallback('esx_testrit:getPlayerInventory', function (inventory)
		  local elements = {}
		  ...
		  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			  ...
			  ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				  ...
				  if count == nil then
					  ...
					  TriggerServerEvent('esx_testrit:putStockItems', itemName, count)
					  menu2.close()
					  menu.close()
					  OpenPutStocksMenu()
				  end
  ...
  AddEventHandler('esx_testrit:hasEnteredMarker', function (zone)
  ...
  AddEventHandler('esx_testrit:hasExitedMarker', function (zone)
  ...
  Citizen.CreateThread(function ()
	  while true do
		  ...
		  if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			  ...
			  TriggerEvent('esx_testrit:hasEnteredMarker', currentZone)
		  end
  
		  if not isInMarker and HasAlreadyEnteredMarker then
			  HasAlreadyEnteredMarker = false
			  TriggerEvent('esx_testrit:hasExitedMarker', LastZone)
		  end
	  end
  end)
		  ...
		  if CurrentAction == nil then
			  ...
			  if IsControlJustReleased(0, Keys['E']) then
				  ...
				  elseif CurrentAction == 'give_back_vehicle' then
					  ESX.TriggerServerCallback('esx_testrit:giveBackVehicle', function(isRentedVehicle)
				  ...
				  elseif CurrentAction == 'resell_vehicle' then
					  ESX.TriggerServerCallback('esx_testrit:resellVehicle', function(vehicleSold)
						  if vehicleSold then
							  ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
