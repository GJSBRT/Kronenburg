...
function HasJob()
	...
	for k, v in ipairs(Config.AllowedJobs) do
		if v.job == PlayerData.job.name then
			if v.grade == nil and v.minGrade ~= nil then 
				if PlayerData.job.grade >= v.minGrade then Result = true end
			elseif v.grade ~= nil and v.minGrade == nil then 
				if PlayerData.job.grade == v.grade then Result = true end
			end
			break
		end
	end
