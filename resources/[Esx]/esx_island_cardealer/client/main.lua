...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_island_cardealer:getCategories', function(categories)
		Categories = categories
	end)

	ESX.TriggerServerCallback('esx_island_cardealer:getVehicles', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_island_cardealer:sendCategories')
AddEventHandler('esx_island_cardealer:sendCategories', function(categories)
	Categories = categories
end)

RegisterNetEvent('esx_island_cardealer:sendVehicles')
AddEventHandler('esx_island_cardealer:sendVehicles', function(vehicles)
	Vehicles = vehicles
end)
...
function OpenBuyLicenseMenu(zone)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_license', {
		...
		if data.current.value == 'yes' then
			ESX.TriggerServerCallback('esx_island_cardealer:buyLicense', function(bought)
				if bought then
					menu.close()
					OpenShopMenu()
				end
...
function OpenShopMenu()
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('esx_island_cardealer: vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				local playerData = ESX.GetPlayerData()
					ESX.TriggerServerCallback('esx_island_cardealer:buyVehicle', function(hasEnoughMoney)
						if hasEnoughMoney then
							...
							ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function(vehicle)
								...
								if Config.EnableOwnedVehicles then
									TriggerServerEvent('esx_island_cardealer:setVehicleOwned', vehicleProps)
								end
...
-- Entered Marker
AddEventHandler('esx_island_cardealer:hasEnteredMarker', function (zone)
...
-- Exited Marker
AddEventHandler('esx_island_cardealer:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('esx_island_cardealer:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_island_cardealer:hasExitedMarker', LastZone)
		end
		...
		if CurrentAction == nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				elseif CurrentAction == 'resell_vehicle' then
					...
					ESX.TriggerServerCallback('esx_island_cardealer:resellVehicle', function(vehicleSold)
						if vehicleSold then
