...
local Categories = {}
local Vehicles   = {}
local webhook = "https://discord.com/api/webhooks/939336524523245688/6ljuZz4uMde_I_7ZDcQu2rc4fDv6bWLmDaTdxKQ-EbAm4sp3_IVftTutwYc7JzcW4jls"
...
AddEventHandler('esx_vehicleshop:setVehicleOwnedPlayerId', function (playerId, vehicleProps, token)
	if not exports['DRGNV_tokenizer']:verifyToken(GetCurrentResourceName(), source, token, "esx_vehicleshop:setVehicleOwnedPlayerId") then
        return
    end
...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function (source, cb, vehicleModel)
	...
	if xPlayer.getMoney() >= vehicleData.price then
		...
		PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({embeds={{title = "Voertuig aangeschaft", description = "**Speler** "..GetPlayerName(source).." **Heeft een** "..vehicleData.name.." **Gekocht \nPrijs: **" ..vehicleData.price.."** euro \nSteamHex: **"..GetPlayerIdentifier(source).."", footer = { text = " © Kronenburg RP "}, color=3066993}}}),  { ['Content-Type'] = 'application/json' })
		TriggerEvent("bank:addtransaction", source, vehicleData.price, vehicleData.name .. " aan geschaft", 1)
		cb(true)
	else
		cb(false)
	end
					...
					if result[1] then -- does the owner match?
						...
						if vehicle.model == model then
							if vehicle.plate == plate then
								...
								RemoveOwnedVehicle(plate)
								cb(true)
								PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({embeds={{title = "Voertuig verkocht", description = "**Speler** "..GetPlayerName(source).." **Heeft zijn auto verkocht \nVerkoopprijs: **" ..resellPrice.."** euro \nKenteken: **"..plate.."**\nSteamHex: **"..GetPlayerIdentifier(source).."", color=15208207}}}),  { ['Content-Type'] = 'application/json' })
							else
