...
ESX.RegisterServerCallback('eden_garage:getVehiclesMecano', function(source, cb)
	MySQL.Async.fetchAll([[SELECT owned_vehicles.*, users.firstname, users.lastname, jobs.label as joblabel FROM owned_vehicles 
		...
		WHERE fourrieremecano = TRUE]], { }, function(result)
		cb(result)
	end)
end)
-- Fin --Recupere les véhicules

ESX.RegisterServerCallback('eden_garage:getVehiclesPrices', function(source, cb, vehicle)
...
AddEventHandler('eden_garage:ChangeStateFrompound', function(vehicleProps, fourrieremecano)
	local _source = source
...
ESX.RegisterServerCallback('eden_garage:checkMoney', function(source, cb, money)
	...
	if xPlayer.getMoney() >= money then
		...
		TriggerEvent("bank:addtransaction", source, money, "Garage Kosten", 1)
		cb(true)
	else
		cb(false)
	end
