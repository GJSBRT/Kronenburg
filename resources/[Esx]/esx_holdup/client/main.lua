...
AddEventHandler('esx_holdup:robberyComplete', function(award)
	holdingUp, store = false, ''
	exports['mythic_notify']:DoHudText('success', _U('robbery_complete', award))
...
AddEventHandler('esx_holdup:startTimer', function()
	...
	Citizen.CreateThread(function()
		while holdingUp do
			Citizen.Wait(7)
			drawTxt(0.86, 1.44, 0.0, 2.85, 0.3, _U('robbery_timer', timer), 255, 255, 255, 255)
		end
	end)
end)
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, Keys['E']) then
							if IsPedArmed(PlayerPedId(), 7) then
								...
								exports['mythic_notify']:DoHudText('error', _U('no_threat'))
							end
						end
					end
				end
