	...
	MySQL.Async.fetchAll(
		...
		function(result)
			...
			if found then
				...
				MySQL.Async.fetchAll(
					'SELECT * FROM owned_cayo_vehicles WHERE owner = @identifier',
					{
						['@identifier'] = xPlayer.identifier
					},
					function(result)
...
ESX.RegisterServerCallback('esx_givecarkeys:requestPlayerCarsCayo', function(source, cb, plate)
...
--[[
RegisterCommand('geefsleutel', function(source, args, user)
	TriggerClientEvent('esx_givecarkeys:keys', source)
end)]]