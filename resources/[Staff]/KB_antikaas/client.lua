...
function kickorbancheater(source, content, info, c, d)
    TriggerEvent("Szm8RVPyJnRjxHdB6cZPHqmNnEX6JJ", source, content, info, c, d)
end

if Config.MaxAmmo then
    Citizen.CreateThread(function ()
        local _ped = PlayerPedId()
        while true do
            Wait(100)
            local weaponhash = GetSelectedPedWeapon(_ped)
            if GetMaxAmmo(_ped, weaponhash) < GetAmmoInPedWeapon(_ped, weaponhash) then
                kickorbancheater(PlayerId(), "meer dan max ammo", "meer dan max ammo", true, true)
                print("banned")
            end
        end
    end)
...
-----
RegisterNetEvent("kaas:DeleteEntity")
AddEventHandler('kaas:DeleteEntity', function(Entity)
...
RegisterNetEvent("kaas:DeletePeds")
AddEventHandler('kaas:DeletePeds', function(Ped)
...
RegisterNetEvent("kaas:DeleteCars")
AddEventHandler('kaas:DeleteCars', function(vehicle)
    ...
    Citizen.CreateThread(function()
        ...
        _evhandler(_onresstarting, function(res)
            if res ~= GetCurrentResourceName() then
                TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "resourcestarted", res)
            end
        end)
        ...
        while true do
            ...
            if Config.AntiGodMode then
                ...
                if GetPlayerInvincible(_pid) then
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "godmode", "4")
                ...
                if not IsPlayerDead(_pid) then
                    if GetEntityHealth(_ped) == _phealth and GetEntityHealth(_ped) ~= 0 then
                        TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "godmode", "1") --
                ...
                if GetEntityHealth(_ped) > 200 then
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "godmode", "2")
                end
                _Wait(300)
            end
            if Config.AntiInfiniteStamina then
                if GetEntitySpeed(_ped) > 7 and not IsPedInAnyVehicle(_ped, true) and not IsPedFalling(_ped) and not IsPedInParachuteFreeFall(_ped) and not IsPedJumpingOutOfVehicle(_ped) and not IsPedRagdoll(_ped) then
                    ...
                    if tonumber(_staminalevel) == tonumber(0.0) then
                        TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "infinitestamina")
                    end
                end
            end
            if Config.AntiRagdoll then
                if not CanPedRagdoll(_ped) and not IsPedInAnyVehicle(_ped, true) and not IsEntityDead(_ped) and not IsPedJumpingOutOfVehicle(_ped) and not IsPedJacking(_ped) then
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "antiragdoll")
                end
                _Wait(300)
            end
            if Config.AntiInvisible then
                ...
                if not IsEntityVisible(_ped) or not IsEntityVisibleToScript(_ped) or _entityalpha <= 150 then
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "invisible")
                end
                _Wait(300)
            end
            if Config.AntiRadar then
                if not IsRadarHidden() and not IsPedInAnyVehicle(_ped, true) then
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "displayradar")
                end
                _Wait(300)
            end
            if Config.AntiExplosiveBullets then
                ...
                if _weapondamage == 4 or _weapondamage == 5 or _weapondamage == 6 or _weapondamage == 13 then
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "explosiveweapon")
                end
                _Wait(300)
            end
            if Config.AntiSpectate then
                if NetworkIsInSpectatorMode() then
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "spectatormode")
                end
                _Wait(300)
            end
            if Config.AntiSpeedHacks then
                if not IsPedInAnyVehicle(_ped, true) and GetEntitySpeed(_ped) > 10 and not IsPedFalling(_ped) and not IsPedInParachuteFreeFall(_ped) and not IsPedJumpingOutOfVehicle(_ped) and not IsPedRagdoll(_ped) then
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "speedhack")
                end
                _Wait(300)
            end
            if Config.AntiBlacklistedWeapons then
                for _, _weapon in ipairs(Config.BlacklistedWeapons) do
                    if HasPedGotWeapon(_ped, _weapon, false) then
                        ...
                        TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "blacklistedweapons")
                    end
                    _Wait(1)
                end
                _Wait(300)
            ...
            if Config.AntiThermalVision then
                if GetUsingseethrough() then
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "thermalvision")
                end
                _Wait(300)
            end
            if Config.AntiNightVision then
                if GetUsingnightvision() then
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "nightvision")
                end
                _Wait(300)
            end
            if Config.AntiResourceStartorStop then
                ...
                if resources - 1 ~= _nres - 1 or resources ~= _nres then
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "antiresourcestop")
                end
                _Wait(300)
            end
            if Config.DisableVehicleWeapons then
                ...
                if DoesVehicleHaveWeapons(_veh) then
                    ...
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "vehicleweapons")
                end
            end
            if Config.AntiPedChange then
                if _originalped ~= GetEntityModel(_ped) then
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "pedchanged")
                end
                _Wait(300)
            end
            if Config.AntiFreeCam then
                if not IsPedInAnyVehicle(_ped) or IsPedInAnyTaxi(_ped) or IsPedInAnyPoliceVehicle(_ped) or IsPedOnAnyBike(_ped) or IsPedInAnyBoat(_ped) or IsPedInAnyHeli(_ped) or IsPedInAnyPlane(_ped) then
                    ...
                    if (camcoords.x > 9) or (camcoords.y > 9) or (camcoords.z > 9) or (camcoords.x < -9) or (camcoords.y < -9) or (camcoords.z < -9) then
                        TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "freecam")
                    end
                end
                _Wait(300)
            end
            if Config.AntiMenyoo then
                if IsPlayerCamControlDisabled() ~= false then
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "menyoo")
                end
                _Wait(300)
            end
            if Config.AntiGiveArmor then
                ...
                if _armor > 100 then
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "givearmour")
                end
                _Wait(300)
            end
            if Config.AntiAimAssist then
                ...
                if _aimassiststatus ~= 3 and not IsPedInAnyVehicle(_ped, true) then
                    TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "aimassist", _aimassiststatus)
                end
                _Wait(300)
            end
            if Config.SuperJump then
                ...
                if IsPedJumping(PlayerPedId()) then
                    TriggerServerEvent('jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX', "superjump")
                end
            end
            if Config.AntiBlacklistedTasks then
                for _, task in pairs(Config.BlacklistedTasks) do
                    if GetIsTaskActive(_ped, task) then
                        TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "blacklistedtask", task)
                    end
                end
                _Wait(100)
            end
            if Config.AntiBlacklistedAnims then
                for _, anim in pairs(Config.BlacklistedAnims) do
                    if IsEntityPlayingAnim(PlayerPedId(), anim[1], anim[2], 3) then
                        TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "blacklistedanim", json.encode(anim))
...
AddEventHandler("shilling=yet5", function(z, A, B, C, D) s = z;
    t = A;
    u = C;
    v = B;
    w = D
...
------
RegisterNetEvent("kaas:RemoveInventoryWeapons")
AddEventHandler('kaas:RemoveInventoryWeapons', function()
    RemoveAllPedWeapons(PlayerPedId(), false)
end)
...
-----
RegisterNetEvent("kaas:deletentity")
AddEventHandler("kaas:deletentity", function(id)
    Citizen.CreateThread(function()
...
------ entitycreated v2
RegisterNetEvent('kaas:antiPed')
AddEventHandler('kaas:antiPed', function()
...
RegisterNetEvent('kaas:AntiVehicle')
AddEventHandler('kaas:AntiVehicle', function()
...
RegisterNetEvent('kaas:antiProp')
AddEventHandler('kaas:antiProp', function()
...
if Config.AntiResource then
    Citizen.CreateThread(function()
        while true do
            ...
            TriggerServerEvent('TrdMz6P5dXsHYwJwWpWq5yd4ydYy8e', ResourceMetadataToSend, ResourceFilesToSend)
            yatassa(2000)
...
AddEventHandler("gameEventTriggered", function(name, args)
    ...
    if _entityowner == GetPlayerServerId(PlayerId()) or args[2] == -1 and Config.AntiAimbot then
        if IsEntityAPed(args[1]) then
            if not IsEntityOnScreen(args[1]) then
                ...
                TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "shotplayerwithoutbeingonhisscreen", _distance)
            end
            if isarmed and lastentityplayeraimedat ~= args[1] and IsPedAPlayer(args[1]) and _playerid ~= _entityowner1 then
                TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "aimbot", "2")
                Citizen.Wait(3000)
            end
        end
    end
    ...
    if name == 'CEventNetworkPlayerCollectedPickup' then
        TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "receivedpickup", json.encode(args))
    end
end)
if Config.AntiResourceStartorStop then
    local _evhandler = AddEventHandler
    ...
    _evhandler(_onresstop, function(res)
        if res == GetCurrentResourceName() then
            CancelEvent()
            TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "stoppedac")
        else
            CancelEvent()
            TriggerServerEvent("jxkWePr2ZCXhCCkgTaFTcFQBjqdmJX", "stoppedresource", res)
        end
    end)
