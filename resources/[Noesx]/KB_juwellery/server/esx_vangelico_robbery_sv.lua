...
ESX.RegisterServerCallback('esx_vangelico_robbery:server:getVerifyToken', function(source, cb)
	local src = source
...
AddEventHandler('esx_vangelico_robbery:gioielli', function(token)
	...
	if not exports['DRGNV_tokenizer']:verifyToken(GetCurrentResourceName(), source, token, "esx_vangelico_robbery:gioielli") then
        return
    end
...
AddEventHandler('lester:vendita', function(token)
	...
	if not exports['DRGNV_tokenizer']:verifyToken(GetCurrentResourceName(), _source, token, "lester:vendita") then
        return
    end
