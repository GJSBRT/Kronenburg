...
Citizen.CreateThread(function()
	...
	while true do
		...
		if distance < 1.5 then
			...
			if golfplaying then
				DisplayHelpText("Druk op ~g~~INPUT_CONTEXT~~s~ om te stoppen.")
			else
				DisplayHelpText("Druk op ~g~~INPUT_CONTEXT~~s~ om te starten met golfen ($100).")
			end
			if (IsControlJustReleased(1, 38)) then
				...
				if (golfplaying) then
					...
					TriggerEvent("customNotification","Druk op E om te slaan, A-D om te draaien, Y om van club te wisselen.")
				end
			end
...
function startGolfHud()
	while golfplaying do
		Citizen.Wait(0)
		DrawRect(0.8,0.93,0.2,0.04,0,0,0,140) -- header
		if golfhole ~= 0 then
			...
			drawGolfTxt(1.2093, 1.391, 1.0,1.0,0.4, "~s~" .. golfstrokes .. "~r~ - ~s~" .. totalgolfstrokes .. "~r~ - ~s~" .. clubname .. "~r~ - ~s~" .. distance .. " m", 255, 255, 255, 255)
		end
	end
end
	...
	while golfstate == 1 do
		...
		if GetVehiclePedIsIn(GetPlayerPed(-1), false) == 0 then
			...
			if distance < 50.0 then
				DisplayHelpText("Ga naar je bal, druk op ~INPUT_PICKUP~ om je bal te laten vallen wanneer je vast zit.")
				if ( IsControlJustReleased(1, 38) ) then
					dropShot()
				end
...
function endShot()
	...
	if distance < 1.5 then
		TriggerEvent("customNotification","Doordat je in de buurt bent word hij goed gekeurd!")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
	if golfstrokes > 12 then
		TriggerEvent("customNotification","Je hebt te vaak geschoten. Volgende keer beter!")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
...
function dropShot()
	...
	while doingdrop do
		...
		if distance < 50.0 and distanceHole > 50.0 then
			DisplayHelpText("Druk op ~INPUT_PICKUP~ Om je bal hier te leggen.")
			if ( IsControlJustReleased(1, 38) ) then
				doingdrop = false
			...
			DisplayHelpText("Druk op ~INPUT_PICKUP~ Om je bal neer te leggen - ~r~ te ver van de bal of te dichtbij de hole.")
		end
	end
end
