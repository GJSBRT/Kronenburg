...
RegisterNetEvent("monteur:succes")
AddEventHandler("monteur:succes", function()
	if group ~= "user" then
		local playerPed = GetPlayerPed(-1)
		local vehicle = GetVehiclePedIsIn(playerPed, false)
		TriggerEvent("pNotify:SendNotification", {text = "Even geduld a.u.b.", type = "info", timeout = 1400, layout = "topRight"})
		exports['sv_laadbalk']:laadbalk('De monteur repareerd je voertuig', 4000)
		...
		TriggerServerEvent("failure:monteur")
		TriggerEvent("pNotify:SendNotification", {text = "De monteur heeft je voertuig gerepareerd.", type = "success", timeout = 1400, layout = "topRight"})
	end
end)
	...
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				TriggerEvent("pNotify:SendNotification", {text = "Ga naar de dichtstbijzijnde monteur.", type = "info", timeout = 3000, layout = "topCenter"})
				fixMessagePos = fixMessagePos + 1
				if fixMessagePos > repairCfg.fixMessageCount then fixMessagePos = 1 end
				TriggerEvent("pNotify:SendNotification", {text = "Actie succesvol uitgevoerd!", type = "success", timeout = 1400, layout = "topRight"})
			else
				notification("~r~Voertuig te erg beschadigd, bel een monteur!")
				TriggerEvent("pNotify:SendNotification", {text = "Actie niet mogelijk!", type = "error", timeout = 1400, layout = "topRight"})
			end
		else
			TriggerEvent("pNotify:SendNotification", {text = "Even geduld a.u.b.", type = "info", timeout = 1400, layout = "topRight"})
			FreezeEntityPosition(vehicle, true)
		...
		TriggerEvent("pNotify:SendNotification", {text = "Actie niet mogelijk!", type = "error", timeout = 1400, layout = "topRight"})
		notification("~y~Je moet in/op een voertuig zitten om een voertuig te repareren.")
	end
end)

RegisterNetEvent('iens:notAllowed')
AddEventHandler('iens:notAllowed', function()
	notification("~r~Je hebt geen rechten om deze commando uit te voeren!")
	TriggerEvent("pNotify:SendNotification", {text = "Actie niet mogelijk!", type = "error", timeout = 1400, layout = "topRight"})
end)
